...
from .utils import center_trim, unfold, capture_init
...
class BLSTM(nn.Module):
    ...
    def forward(self, x):
        B, C, T = x.shape
        y = x
        framed, nframes, width, stride = False, 0, 0, 0
...
class LocalState(nn.Module):
    ...
    def forward(self, x):
        ...
        freq_kernel = None
        if self.nfreqs:
            assert self.query_freqs is not None
            periods = torch.arange(1, self.nfreqs + 1, device=x.device, dtype=x.dtype)
            freq_kernel = torch.cos(2 * math.pi * delta / periods.view(-1, 1, 1))
            freq_q = self.query_freqs(x).view(B, heads, -1, T) / self.nfreqs**0.5
            dots += torch.einsum("fts,bhfs->bhts", freq_kernel, freq_q)
        if self.ndecay:
            assert self.query_decay is not None
        ...
        if self.nfreqs:
            assert freq_kernel is not None
