...
class HTDemucs(nn.Module):
    ...
    def __init__(
        ...
        for index in range(depth):
            ...
            enc = HEncLayer(chin_z, chout_z, dconv=dconv, context=context_enc, **kw)
            if freq:
                tenc = HEncLayer(
                    chin,
                    chout,
    ...
    def _ispec(self, z, length: int = 0, scale: int = 0):
        hl = int(self.hop_length // (4**scale))
        z = F.pad(z, (0, 0, 0, 1))
        z = F.pad(z, (2, 2))
        pad = hl // 2 * 3
    ...
    def _wiener(self, mag_out, mix_stft, niters: int):
    ...
    def valid_length(self, length: int):
        ...
        if training_length < length:
            raise ValueError(
                f"Given length {length} is longer than " f"training length {training_length}"
            )
        return training_length

    def forward(self, mix: Tensor):
        length = mix.shape[-1]
        length_pre_pad = 0
        training_length = 0
        ...
        saved: List[Tensor] = []  # skip connections, freq.
        saved_t: List[Tuple[bool, Tensor]] = []  # inject and skip connections, time.
        ...
        pres: List[Tensor] = []  # saved pre-activation, freq.
