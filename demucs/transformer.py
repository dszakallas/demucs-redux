...
def create_sin_embedding_cape(
    ...
    if augment:
        delta = torch.rand(size=[1, batch_size, 1]) * 2 * max_global_shift - max_global_shift
        delta_local = (
            torch.rand(size=[length, batch_size, 1]) * 2 * max_local_shift - max_local_shift
        )
        log_scale = math.log(max_scale)
        log_lambdas = torch.rand(size=[1, batch_size, 1]) * 2 * log_scale - log_scale
...
class MyTransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self.norm_out = None
        if self.tel.norm_first & norm_out:
    ...
    def forward(
        ...
        if self.tel.norm_first:
            x = x + self.gamma_1(
                self.tel._sa_block(self.tel.norm1(x), src_mask, src_key_padding_mask)
            )
            x = x + self.gamma_2(self.tel._ff_block(self.tel.norm2(x)))
...
class TransformerOrCrossEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        if isinstance(self.encoder, MyTransformerEncoderLayer):
...
class CrossTransformerEncoder(nn.Module):
    def __init__(
        ...
        for idx in range(num_layers):
            if idx % 2 == self.classic_parity:
                l = MyTransformerEncoderLayer(**kwargs_classic_encoder)
                lt = MyTransformerEncoderLayer(**kwargs_classic_encoder)
            else:
                l = CrossTransformerEncoderLayer(**kwargs_cross_encoder)
                lt = CrossTransformerEncoderLayer(**kwargs_cross_encoder)
            self.layers.append(TransformerOrCrossEncoderLayer(l))
            self.layers_t.append(TransformerOrCrossEncoderLayer(lt))
    ...
    def forward(self, x, xt):
        ...
        pos_emb_2d = pos_emb_2d.permute(0, 3, 2, 1).reshape(B, -1, C)
        # x = rearrange(x, "b c fr t1 -> b (t1 fr) c")
        x = x.permute(0, 3, 2, 1).reshape(B, -1, C)
        x = self.norm_in(x)
        x = x + self.weight_pos_embed * pos_emb_2d

        B, C, T2 = xt.shape
        # xt = rearrange(xt, "b c t2 -> b t2 c")  # now T2, B, C
        xt = xt.permute(0, 2, 1)  # now T2, B, C
        ...
        for l, lt in zip(self.layers, self.layers_t):
            old_x = x
            x = l(x, xt)
            xt = lt(xt, old_x)
            # if idx % 2 == self.classic_parity:
    ...
    def _get_pos_embedding(
        ...
        if self.emb == "sin":
            ...
            shift = int(torch.randint(self.sin_random_shift + 1, (1,)))
            ...
            raise ValueError(f"Unknown positional embedding {self.emb}")
